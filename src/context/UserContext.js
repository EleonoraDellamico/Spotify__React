...
export const UserProvider = ({ children }) => {
	...
	const nextSongFromQueue = () => {
		const auxUser = { ...user };
		const nextSongPosition = auxUser.queue.findIndex((song) => song.id === auxUser.currentSong) + 1;
		if (nextSongPosition) {
	...
	const previousSongFromQueue = () => {
		const auxUser = { ...user };
		const previousSongPosition = auxUser.queue.findIndex((song) => song.id === auxUser.currentSong) - 1;
	...
	const playSongFromQueue = (song) => {
		const auxUser = { ...user };
		const songPosition = auxUser.queue.findIndex((song) => song.id === auxUser.currentSong);
		if (songPosition >= 0) {
